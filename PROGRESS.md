...
- [ ] Custom SMTP via API — **on hold, deliberate decision (2026-08-27):** Mailgun needs `mg.xanalife.com` DNS-verified in GoDaddy, and roystone doesn't have GoDaddy access for `xanalife.com`. Rather than block on IT/CEO, staying on **Supabase's built-in mailer** for now — accepted tradeoff: ~2 emails/hour shared project-wide, no branded templates (free tier locks template edits without custom SMTP). Script (`scripts/set-smtp-and-template.mjs`) + branded logo'd templates for both **magic link** (`scripts/email-templates/magic-link.html`) and **invite** (`scripts/email-templates/invite.html`, added 2026-08-28 — matches `api/admin-users.js`'s invite flow) are built and ready — just need `mg.xanalife.com` DNS access resolved (Cloudflare NS delegation is the cheapest path — only needs 2 NS records from whoever owns GoDaddy, then self-service after) then fill `.env.local` (`MAILGUN_SMTP_USER/PASS`, `MAILGUN_SENDER_EMAIL/NAME`) and run it. Verify with `node scripts/check-auth-config.mjs`.
