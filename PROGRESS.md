...
- [ ] Custom SMTP via API — **switched from Mailgun back to Office 365 (2026-08-28):** roystone has confirmed no GoDaddy access for `xanalife.com` and won't get any — closes off Mailgun entirely (even the Cloudflare-NS-delegation idea needed one GoDaddy touch). Office 365 needs zero new DNS since `xanalife.com` mail is already verified there — just the noreply mailbox's SMTP credentials. **Gotcha to watch for:** Microsoft disables basic SMTP AUTH tenant-wide by default now — IT needs to explicitly enable it for the noreply mailbox (`Set-CASMailbox -Identity noreply@xanalife.com -SmtpClientAuthenticationDisabled $false`), or the password alone will fail with an auth error. Script (`scripts/set-smtp-and-template.mjs`, now targets `smtp.office365.com:587`) + branded logo'd templates for **magic link** (`scripts/email-templates/magic-link.html`) and **invite** (`scripts/email-templates/invite.html`) are ready — just need `.env.local` filled (`OFFICE365_SMTP_USER/PASS`, `OFFICE365_SENDER_NAME`) and the script run. Verify with `node scripts/check-auth-config.mjs`. Currently still on Supabase's built-in mailer (~2 emails/hour shared project-wide, no branded templates — free tier locks template edits without custom SMTP).
